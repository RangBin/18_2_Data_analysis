...
# - second model uses RELU activation function
model_relu = keras.models.Sequential()
model_relu.add(keras.layers.Dense(units=96, input_dim = hu_final_train.shape[1], activation = 'relu'))
model_relu.add(keras.layers.Dropout(0.5))
model_relu.add(keras.layers.Dense(30,activation=tf.nn.relu))
model_relu.add(keras.layers.Dense(24,activation=tf.nn.relu))
model_relu.add(tf.keras.layers.Dense(6, activation=tf.nn.softmax))
...
history_relu = model_relu.fit(hu_final_train.values, oh_training_labels, epochs = 120, batch_size = 30, callbacks = early_stopping,
                    verbose = 2, validation_split = .15, shuffle=True)

# print evaluation result - using first set's test dataset
val_loss, val_acc = model_relu.evaluate(hu_final_test.values,oh_testing_labels)
print("Loss : "+str(val_loss))
print("Accuracy : "+str(val_acc))

# plot - confusion matrix
predictions = model_relu.predict(hu_final_test)
...
sp_loss, sp_acc = model_relu.evaluate(sp_final_train.values, sp_oh_training_labels)
print("++ Evaluation on another set")
print("Loss : "+str(sp_loss))
print("Accuracy : "+str(sp_acc))

# plot confusion matrix on second dataset
sp_predictions = model_relu.predict(sp_final_train)
